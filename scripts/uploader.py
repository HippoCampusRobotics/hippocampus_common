...
class firmware(object):
    ...
    def __parseline(self, line):
        ...
        if (command == 4 and ord(binstr[0]) == 0x02
                and (ord(binstr[1]) << 8) + ord(binstr[2]) == 0x0000):
            self.upperaddress = (ord(binstr[4]) << 8) + ord(binstr[5])
            self.bankingDeteted = True
        elif (command == 0):
            address = (ord(binstr[1]) << 8) + ord(
                binstr[2]) + (self.upperaddress << 16)
            bytes = bytearray(binstr[4:])
            if self.upperaddress in self.sanity_check:
                self.sanity_check[self.upperaddress][0] += len(bytes)
                if self.sanity_check[self.upperaddress][1] > (
                        ord(binstr[1]) << 8) + ord(binstr[2]):
                    self.sanity_check[self.upperaddress][1] = (
                        ord(binstr[1]) << 8) + ord(binstr[2])
                if self.sanity_check[self.upperaddress][2] < (
                        ord(binstr[1]) << 8) + ord(binstr[2]) + len(bytes):
                    self.sanity_check[self.upperaddress][2] = (
                        ord(binstr[1]) << 8) + ord(binstr[2]) + len(bytes)
            else:
                self.sanity_check[self.upperaddress] = []
                self.sanity_check[self.upperaddress].append(len(bytes))
                self.sanity_check[self.upperaddress].append(
                    (ord(binstr[1]) << 8) + ord(binstr[2]))
                self.sanity_check[self.upperaddress].append(
                    (ord(binstr[1]) << 8) + ord(binstr[2]) + len(bytes))
...
class uploader(object):
    ...
    def __init__(self,
        ...
        if use_mavlink:
            ...
            self.port = mavutil.MavlinkSerialPort(portname,
                                                  115200,
                                                  devnum=mavport,
                                                  timeout=3,
                                                  debug=debug)
    ...
    def __recv(self, raise_error=True):
        ...
        c = self.port.read(1)
        if (len(c) < 1):
            self.debug("timeout waiting for data (%.2f seconds)" %
    ...
    def __getSync(self, raise_error=True):
        ...
        if c is None:
            return False
        if (c != self.INSYNC):
            self.debug("unexpected 0x%x instead of INSYNC (0x%x)" %
                       (ord(c), ord(self.INSYNC)))
            if raise_error:
                raise RuntimeError("unexpected 0x%x instead of INSYNC (0x%x)" %
                                   (ord(c), ord(self.INSYNC)))
            return False
        c = self.__recv()
        if (c != self.OK):
            self.debug("unexpected 0x%x instead of OK (0x%x)" %
                       (ord(c), ord(self.OK)))
            if raise_error:
                raise RuntimeError("unexpected 0x%x instead of OK (0x%x)" %
                                   (ord(c), ord(self.OK)))
            return False
    ...
    def __erase(self, erase_params=False):
        ...
        self.__getSync()
        if (erase_params):
            self.__send(uploader.PARAM_ERASE + uploader.EOC)
            self.__getSync()
    ...
    def __set_address(self, address, banking):
        if (banking):
            ...
            self.__send(uploader.LOAD_ADDRESS + chr(address & 0xff) +
                        chr((address >> 8) & 0xff) +
                        chr((address >> 16) & 0xff) + uploader.EOC)
        else:
            self.__send(uploader.LOAD_ADDRESS + chr(address & 0xff) +
                        chr(address >> 8) + uploader.EOC)
        self.__getSync()

    # send a PROG_FLASH command to program one byte
    def __program_single(self, data):
        self.__send(uploader.PROG_FLASH + chr(data) + uploader.EOC)
        self.__getSync()
    ...
    def __verify(self, data):
        self.__send(uploader.READ_FLASH + uploader.EOC)
    ...
    def __verify_multi(self, data):
        self.__send(uploader.READ_MULTI + chr(len(data)) + uploader.EOC)
        for i in data:
        ...
        for address in sorted(code.keys()):
            ...
            for bytes in groups:
                if (not self.__verify_multi(bytes)):
                    raise RuntimeError("Verification failed in group at 0x%x" %
                                       address)
                count += len(bytes)
                self.progress(count, total)
                self.progress(count, total)
    ...
    def identify(self):
        self.__send(uploader.GET_DEVICE + uploader.EOC)
    ...
    def setParams(self, args):
        ...
        setParams = {
            'S1': int(args.setBaudrate / 1000),  # SERIAL_SPEED
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Firmware uploader for the SiK radio system.")
    parser.add_argument('firmware',
                        action="store",
                        help="Firmware file to be uploaded")
    parser.add_argument('--port', action="store", help="port to upload to")
    parser.add_argument('--resetparams',
                        action="store_true",
                        help="reset all parameters to defaults")
    parser.add_argument("--baudrate", type=int, default=57600, help='baud rate')
    parser.add_argument('--forceBanking',
                        action="store_true",
                        help="force the programmer to use 24bit addressing")
    parser.add_argument("--mavlink",
                        action='store_true',
                        default=False,
                        help='update over MAVLink')
    parser.add_argument("--mavport",
                        type=int,
                        default=0,
                        help='MAVLink port number')
    parser.add_argument("--debug", type=int, default=0, help='debug level')

    parser.add_argument("--setParams",
                        action='store_true',
                        default=False,
                        help='set parameters after uploading firmware')
    parser.add_argument("--onlyParams",
                        action='store_true',
                        default=False,
                        help='set parameters without uploading')
    parser.add_argument("--setBaudrate",
                        type=int,
                        default=57600,
                        help='set baud rate')
    parser.add_argument("--setNetId", type=int, default=12, help='network id')
    parser.add_argument("--setRTSCTS",
                        action='store_true',
                        default=False,
                        help='Hardware flow controll')
    parser.add_argument("--setNodeCount",
                        type=int,
                        default=2,
                        help='total nodes in a network')
